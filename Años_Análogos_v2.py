...
SSSTA_12 = pd.read_csv("dataSSTA_12.csv",usecols =[i for i in cols_SSTA if i != 'Unnamed: 0'])
SSTA_12 = SSTA_12[SSTA_12['YEAR'].notna()]
...
SSTA_3 = SSTA_3[SSTA_3['YEAR'].notna()]
...
SSTA_4 = SSTA_4[SSTA_4['YEAR'].notna()]
...
SSTA_34 = SSTA_34[SSTA_34['YEAR'].notna()]
...
AtlTROP = AtlTROP[AtlTROP['YEAR'].notna()]
...
def InputOsc():
    while True:
        oscilacion = input("\n" + "Por favor ingrese la oscilación que desea usar (AMO, AO, MEI, ONI, NAO, PDO, TNA, SSTA_12, SSTA_3, SSTA_4, SSTA_34, AtlTROP, SAtl, NAtl, CAR, WHWP, PNA, SOI, AMO_CSU): ")
        try:        
            words = [str(x) for x in oscilacion.split()] 
            if all(word in Oscilaciones for word in words):
                print("\n" + "***** Oscilaciones leídas correctamente *****" + "\n")
                break
            else:
                print("\n" + "***** La oscilación ingresada es incorrecta, inténtelo de nuevo. *****" + "\n")
                oscilacion = InputOsc()
                break
        except ValueError:
...
def InputMes():
    while True:
        mes = input("Por favor ingrese el número del mes que desea utilizar: " + "\n")
        try:        
            val = int(mes)
            if (val > 0) & (val <= 12):
                mes = int(mes)
                ...
                print("\n" + "***** El valor ingresado tiene que ser mayor a 0 y menor o igual que 12, inténtelo de nuevo. *****" + "\n")
                continue
        except ValueError:
            print("Dato ingresado incorrecto.")
            continue
...
print("Bienvenidos, este programa se encarga de calcular los años análogos de una manera automatizada a partir de las siguientes oscilaciones AMO, AO, MEI, ONI, NAO, PDO, TNA, SSTA_12, SSTA_3, SSTA_4, SSTA_34, AtlTROP, SAtl, NAtl, CAR, WHWP, PNA, SOI, AMO_CSU." + "\n \n" + "Nota: Antes de continuar leer el manual del usuario." + "\n \n" + "Este código creado por el meteorólogo Anthony Segura García de la Universidad de Costa Rica y del Departamento Red Meteorológica y Procesamiento de Datos del Instituto Meteorológico Nacional de Costa Rica." + "\n \n" +  "A continuación se le solicitarán las opciones necesarias para el cálculo: " + "\n")
